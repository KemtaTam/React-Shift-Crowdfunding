...
const RegistrationForm = memo((props) => {
	const activeLink = ({ isActive }) =>
		isActive ? s.form_navigate_el_active : s.form_navigate_el; //для подсветки активной ссылки
		...
		<Formik
			...
			validationSchema={Yup.object({
				email: Yup.string()
					...
					.max(255, "Email должен содержать не более 255 символов"),
				password: Yup.string()
					.min(5, "Должен содержать от 5 до 255 символов")
					.max(255, "Должен содержать от 5 до 255 символов")
					.required("Обязательно")
					.matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{2,})/,
					"Должен содержать от 5 до 255 символов, одну большую букву, одну маленькую, одну цифру и один специальный символ"
					),
