	...
	return (
		<div className={s.projectEl}>
			...
			<div className={s.summ_wrapper}>
				<div className={s.summ}>{projectData?.collectedAmount + '/' + projectData?.requiredAmount + '₽'}</div>
			</div>
			<NavLink to={`/projects/${projectData?.id}`}><Button className={s.bDonate} type='primary'>Поддержать</Button></NavLink>
		</div>
	)
}
